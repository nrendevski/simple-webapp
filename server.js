...
<head>
    ...
    <style>
        body { font-family: Arial; background:#f0f2f5; padding:40px; display:flex; justify-content:center;}
        .container { background:white; padding:30px; width:850px; border-radius:14px; box-shadow:0 4px 15px rgba(0,0,0,0.15);}
        ...
        button{padding:10px 20px;background:#0066ff;border:none;color:white;border-radius:6px;cursor:pointer;font-weight:bold;}
        button:hover{background:#004ecc;}
        .pdf-btn{display:inline-block;padding:8px 15px;background:#28a745;color:white;border-radius:6px;text-decoration:none;font-size:14px;margin-bottom:20px;}
        .pdf-btn:hover{background:#1d7e34;}
        table{width:100%;border-collapse:collapse;margin-top:10px;}
        th{background:#0066ff;color:white;padding:12px;}
        td{padding:12px;border-bottom:1px solid #ddd;}
        .action-btn{padding:6px 12px;border-radius:5px;text-decoration:none;font-size:13px;color:white;margin-right:8px;}
...
app.get("/delete/:id", (req, res) => {
    db.run("DELETE FROM users WHERE id = ?", [req.params.id], () => res.redirect("/"));
});
...
<style>
body{font-family:Arial;background:#eef1f5;display:flex;justify-content:center;padding-top:60px;}
.card{background:white;padding:30px;width:400px;border-radius:14px;box-shadow:0 4px 15px rgba(0,0,0,0.15);}
h1{text-align:center;}
form{display:flex;flex-direction:column;gap:15px;}
input{padding:12px;border:1px solid #bbb;border-radius:6px;}
button{padding:12px;background:#007bff;border:none;color:white;border-radius:6px;font-size:16px;}
...
app.get("/api/power/:num", (req, res) => {
    ...
    if (isNaN(n)) return res.status(400).json({ error: "Input must be a number" });
    res.json({ input: n, result: Math.pow(2, n) });
});

// -----------------------------------
// EXPORT PDF (prompts Save dialog)
// -----------------------------------
